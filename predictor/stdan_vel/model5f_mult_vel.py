...
class GDEncoder(nn.Module): 
    def __init__(self, args):
        ...
        #  attention embeding : what is qff? 
    ...
    def forward(self, hist, nbrs, mask, va, nbrsva, lane, nbrslane, cls, nbrscls):
        ...
        hist_enc = self.activation(self.linear1(hist)) # MTP ELU(MLP(x,W)) = h (motion encoding)
        hist_hidden_enc, (_, _) = self.lstm(hist_enc)  # For temporal correlation (T, B, D)
        hist_hidden_enc = hist_hidden_enc.permute(1, 0, 2) # permute : 차원 간의 순서 바꾸기 , (B, T, D)
        
        # nbrs agent
        nbrs_enc = self.activation(self.linear1(nbrs)) # ELU(MLP(x,w))
        nbrs_hidden_enc, (_, _) = self.lstm(nbrs_enc) # LSTM(ELU(MLP(x,w))) (T, B, D)

        mask = mask.view(mask.size(0), mask.size(1) * mask.size(2), mask.size(3))
        mask = repeat(mask, 'b g s -> t b g s', t=self.in_length) # t라는 시간 차원을 추가로 생성 
        # scatter grid embeding
        soc_enc = t.zeros_like(mask).float() # 0 tensor 생성
        soc_enc = soc_enc.masked_scatter_(mask, nbrs_hidden_enc) # put into Social grid (H_t)

        query = self.qff(hist_hidden_enc) # query of ego, h_t^0
        ...
        # keys : (128, 124, 48, 40) values : (128, 124, 40, 48)
        # keys 과정 : ([31, 128, 40, 192]) -> ([31,128,40,48],[31,128,40,48],[31,128,40,48],[31,128,40,48]) -> (124,128,40,48) -> (124,128,48,40)
        ...
        values = t.matmul(a, values) # -> [128, 124, 1, 48]
        values = t.cat(t.split(values, int(hist.shape[0]), 1), -1) # -> [128, 31, 1, 192]

        values = values.squeeze(2) # -> ([128, 31, 192])

        # gate
        spa_values, _ = self.first_glu(values) # [128, 31, 64]

        # Residual connection
        values = self.addAndNorm(hist_hidden_enc, spa_values) # Output of SI module, \bar(H)^0 
        spa_values = values
        ...
        # Residual connection,
        if self.use_spatial:
            # values = self.addAndNorm(spa_values, spa_values, time_values)
            values = self.addAndNorm(hist_hidden_enc, spa_values, time_values)
        else:
            # print(f'values : {values.shape}')
            # print(f'time_values : {time_values.shape}')
            # values = self.addAndNorm(spa_values, time_values) # 1119 : 이게 더 맞는 구현 아닌가?
...
class Decoder(nn.Module): # Decoding to multi-modal future trajectory
    def __init__(self, args):
...
class Generator(nn.Module): # 가중치 초기화를 위한 모듈
    def __init__(self, args):
    ...
    def forward(self, values, lat_enc, lon_enc):
        # 选择mapping -> mapping 선택
        maneuver_state = values[:, -1, :] # NLL..
        ...
        lat_pred = F.softmax(self.op_lat(maneuver_state), dim=-1) # (batch,3)
        lon_pred = F.softmax(self.op_lon(maneuver_state), dim=-1)
        
        if self.train_flag: # 학습땐 <single-modal, GT> 간의 loss 계산 
            if self.use_true_man:
                ...
                lat_man = lat_enc_tmp.scatter_(1, lat_man, 1) # scatter_(dim, index, value), dim : index가 행인지 열인지 결정, index : 위치, value, 값
                lon_man = lon_enc_tmp.scatter_(1, lon_man, 1)
                
            index = t.cat((lat_man, lon_man), dim=-1).permute(-1, 0) # (len(lat+lon),batch)
            mapping = F.softmax(t.matmul(self.mapping, index).permute(2, 1, 0), dim=-1) # intention weight in IFF matrix 
            dec = t.matmul(mapping, values).permute(1, 0, 2)
        
            if self.use_maneuvers: # True이면 -> multi-modal -> 학습 중엔 uni-modal
                fut_pred = self.Decoder(dec, lat_enc, lon_enc) # (f_horizon, batch, 5)
                return fut_pred, lat_pred, lon_pred
            else: 
                fut_pred = self.Decoder(dec, lat_pred, lon_pred)
                return fut_pred, lat_pred, lon_pred
        else: # output multi modal, 어차피 
            out = []
