...
class GDEncoder(nn.Module): 
    ...
    def forward(self, hist, nbrs, mask, va, nbrsva, lane, nbrslane, cls, nbrscls):
        ...
        hist_hidden_enc, (_, _) = self.lstm(hist_enc)  # For temporal correlation
        hist_hidden_enc = hist_hidden_enc.permute(1, 0, 2) # permute : 차원 간의 순서 바꾸기 
        # nbrs agent
        nbrs_enc = self.activation(self.linear1(nbrs)) # ELU(MLP(x,w))
        nbrs_hidden_enc, (_, _) = self.lstm(nbrs_enc) # LSTM(ELU(MLP(x,w)))
        mask = mask.view(mask.size(0), mask.size(1) * mask.size(2), mask.size(3))
        mask = mask.unsqueeze(0).repeat(self.in_length, 1, 1, 1) # t라는 시간 차원을 추가로 생성 
        # scatter grid embeding
        soc_enc = t.zeros_like(mask).float() # 0 tensor 생성
        soc_enc = soc_enc.masked_scatter_(mask, nbrs_hidden_enc) # putinto Social grid (H_t)
...
class Generator(nn.Module): # 가중치 초기화를 위한 모듈
    ...
    def forward(self, values, lat_enc, lon_enc):
        ...
        if self.train_flag:
            ...
            out = []
            for k in range(self.lon_length): # constant, decel, accel 
                for l in range(self.lat_length): # LK, LC, RC
                    ...
                    out.append(fut_pred.detach().cpu().numpy())
                    
                    
            return out, lat_pred.detach().cpu().numpy(), lon_pred.detach().cpu().numpy() # ? ,(1,3),(1,3)
